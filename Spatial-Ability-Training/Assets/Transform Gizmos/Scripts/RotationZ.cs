...
namespace TransformGizmos
{
    public class RotationZ : MonoBehaviour, IGizmoTransforms
    {
        ...
        {
            m_totalDist = 0;
            (m_initialMousePosition, m_lastProjectedMousePosition, m_previousMousePosition, m_tangent, m_vertices) = Rotation.Instance.MouseDownCode(transform.up, axis: 2);
        }
        ...
        {
            (m_totalDist, m_lastProjectedMousePosition, m_previousMousePosition) = Rotation.Instance.MouseDragCode(m_initialMousePosition, m_previousMousePosition, m_tangent, m_lastProjectedMousePosition, m_totalDist, m_vertices, axis: 2);
        }
    }
}
